...
void DelOddCopEven(Node* headPtr)
{
    ...
    while (currNode != 0)
    {
        if (currNode->data % 2 != 0) //checks if odd and del
        {
            if (currNode == headPtr) //if head needs to be del
            {
            ...
            else
            {
                ...
                delete tempNode;
                prevNode->link = currNode; //connect prevNode to currNode
                
            }
        }
        else //duplicate even
        {
            Node* dupEven = new Node;
            dupEven->data = currNode->data;
            dupEven->link = currNode->link;
            currNode->link = dupEven;
            prevNode = dupEven;
            currNode = dupEven->link; //prevents infinite loop of evens
        }
    }
}
